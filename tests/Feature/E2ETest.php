...
function createUserWithPreferences(array $overrides = []): User
{
...
function createFixedEvents(User $user, array $events): void
{
...
function createScheduledTodo(User $user, array $overrides = []): TodoItem
{
    return TodoItem::create(array_merge([
        'user_id'            => $user->id,
        'title'              => 'Test task',
        'completed'          => false,
        'priority'           => 3,
...
function generateSchedules($testCase, User $user): \Illuminate\Support\Collection
{
    $testCase->actingAs($user)->post('/schedules/generate')->assertRedirect();

    return OptimizedSchedule::where('user_id', $user->id)->get()->keyBy('type');
}
